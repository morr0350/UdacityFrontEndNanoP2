...
var work= {
    ...
    "display": function () {
        ...
        for (var item in this.jobs) {
            ...
            var formattedworkTitleElem = HTMLworkTitle.replace("%data%", this.jobs[item].title);
            var formattedworkEmployer = HTMLworkEmployer.replace("%data%", this.jobs[item].employer);
            var formattedworkDates = HTMLworkDates.replace("%data%", "Start: " + this.jobs[item].dates.start + ", End: " + this.jobs[item].dates.end);
            var formattedworkLocation = HTMLworkLocation.replace("%data%", this.jobs[item].location);
            var formattedDescription = HTMLworkDescription.replace("%data%", this.jobs[item].description);
...
var education = {
	...
	"display": function () {
		...
		for (var item in this.schools) {
			...
			var formattedSchoolName = HTMLschoolName.replace("%data%",  this.schools[item].name);
			var formattedSchoolLocation = HTMLschoolLocation.replace("%data%",  this.schools[item].location);
			var formattedSchoolDegree = HTMLschoolDegree.replace("%data%",  this.schools[item].degree);
			var formattedSchoolMajor = HTMLschoolMajor.replace("%data%",  this.schools[item].major.toString());
			var formattedSchoolDates = HTMLschoolDates.replace("%data%",  this.schools[item].dates);
		...
		for (var item in this.onlineCourses) {

			var onlineSchoolDiv = $(HTMLschoolStart);
			educationDiv.append(onlineSchoolDiv);

			var formattedOnlineTitle = HTMLonlineTitle.replace("%data%",  this.onlineCourses[item].title);
			var formattedOnlineSchool= HTMLonlineSchool.replace("%data%",  this.onlineCourses[item].school);
			var formattedOnlineDate = HTMLonlineDates.replace("%data%",  this.onlineCourses[item].date);
			var formattedOnlineUrl = HTMLonlineURL.replace("%data%",  this.onlineCourses[item].url);
...
var projects = {
	...
	"display": function() {
        for (var item in this.projects) {
            ...
            var formattedTitle = HTMLprojectTitle.replace("%data%", this.projects[item].title);
            var formattedDates = HTMLprojectDates.replace("%data%", this.projects[item].dates);
            var formattedDescription = HTMLprojectDescription.replace("%data%", this.projects[item].description);
            var formattedImage = HTMLprojectImage.replace("%data%", this.projects[item].image);
